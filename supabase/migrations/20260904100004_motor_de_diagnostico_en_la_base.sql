-- Motor de diagnóstico en el servidor, antes calculado en el navegador con códigos y
-- precios inventados. Solo recomienda variantes reales de product_variants; si no hay
-- producto para el caso, pide visita técnica. El criterio técnico se porta del motor anterior.

-- Sin ceros de relleno ni el punto final que deja to_char («85.»).
...
-- Null si el producto no existe, está inactivo o no tiene rendimiento (herramientas).
-- La presentación se elige por menor costo que cubra los galones, no por tamaño.
...
begin
  ...
  -- Sin presentación con volumen no se puede cotizar.
...
begin
  ...
  -- Sin área no se calcula; antes se asumían 85 m² en silencio.
  if v_area <= 0 then
    return jsonb_build_object(
  ...
  if v_industrial then
    ...
    -- Sin la advertencia de «anticorrosivo antes de 4 horas»: este 3 en 1 se aplica
    -- directamente sobre óxido firme.
    v_linea := public.material_de_diagnostico('PNT-MET-006', 'Acabado Arquitectónico', 2, v_area);
    if v_linea is not null then v_lineas := v_lineas || v_linea; end if;

  -- Rama propia: antes la madera caía en vinilo de interior.
  elsif v_madera then
  ...
  -- Advertencias independientes de la línea elegida.
  if v_hongos then
  ...
  -- Sin producto en catálogo no se inventa uno.
