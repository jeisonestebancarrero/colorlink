-- Configuración del asistente con IA. La llave solo la lee la función de borde y nunca
-- se devuelve; el modelo redacta sobre datos ya consultados, y si falla se responde
-- con reglas.
...
-- Revocar una columna sobre un GRANT de tabla no recorta nada: se revoca la tabla
-- y se concede la lista de columnas sin la llave.
...
as $$
  select case when public.is_staff() then
    ...
    -- Al cliente solo se le dice si hay IA, nunca el proveedor ni el modelo.
