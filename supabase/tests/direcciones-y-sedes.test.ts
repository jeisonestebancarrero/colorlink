...
/**
 * Direcciones, sedes y destinatario: la dirección es privada, no se puede pedir a una
 * sede ajena, la dirección de la sede la pone el servidor, el envío exige dirección,
 * municipio oficial y destinatario, y DIVIPOLA es de lectura pública. La base queda como estaba.
 */
...
describe.skipIf(!disponible)('Direcciones, sedes y quién recibe', () => {
  ...
  afterAll(async () => {
    // Se borra con service_role: el cliente no puede borrar pedidos y ensuciaría bandeja y analítica.
  ...
  it('al elegir sede, la dirección la pone el SERVIDOR y no el navegador', async () => {
    ...
    // Hace falta un carrito con algo para que el pedido se cree.
    ...
    // Gana la dirección de la sede, no la falsa.
    ...
    // Destinatario guardado y normalizado: mayúsculas, documento sin puntos, teléfono con indicativo.
